...
class EmbeddingVectorStorage:
    ...
    def include_documents(self, documents: list[Document], size_of_batch=41666, should_verbose: bool = False,
        ...
        batches = [documents[i:i + size_of_batch] for i in range(0, len(documents), size_of_batch)]

        for batch in tqdm(batches) if should_verbose else batches:
            self.storage_of_vector.add_documents(documents=batch, verbose=should_verbose)
